...
from app import db
from app.business.marketplace.forms import SProductForm, SShippingForm, SShippingMethodForm
from app.business.marketplace.apis import *
...
def order(step):
    ...
    if request.method == 'POST':
        if step == 1:
            ...
            if cart_instance.step != 2:
                flash("Cart Changed since last interaction, please start over", "error")
                return redirect(url_for('marketplace.order', step=cart_instance.step))
            form = SShippingForm()
            valid = True
            ...
            if form.validate_on_submit():
                cart_details = cart_instance.cart_details
                if not cart_details:
                    ...
                    cart_details.first_name = form.first_name.data
                    cart_details.last_name = form.last_name.data
                    cart_details.email = form.email.data
                    cart_details.mobile_phone = form.mobile_phone.data
                    cart_details.state = form.state.data
                    cart_details.zip = form.zip.data
                    cart_details.country = form.country.data
                    cart_details.city = form.city.data
            ...
            try:
                user_informed = float(request.form['price_paid'])
            except:
                user_informed = 0
            description = "Payment for buying from Mediville Markeplace for order: " + order_number
            if not order_pay_amount == price_to_pay == user_informed:
        ...
        if step == 2:
            ...
            cart_details = cart_instance.cart_details
            if cart_details:
                form.first_name.data = cart_details.first_name
                form.last_name.data = cart_details.last_name
                form.email.data = cart_details.email
                form.mobile_phone.data = cart_details.mobile_phone
                form.state.data = cart_details.state
                form.zip.data = cart_details.zip
                form.country.data = cart_details.country
                form.city.data = cart_details.city
