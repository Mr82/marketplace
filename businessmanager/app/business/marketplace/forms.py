...
##    min_order_quantity = IntegerField('Min number of units per order e.g 1', default=1, validators=[Optional()])
##    length = FloatField('Length in numbers only e.g 0.99', validators=[Optional()])
##    weight = FloatField('Weight in numbers only e.g 0.21', validators=[Optional()])
##    height = FloatField('Height in numbers only e.g 10', validators=[Optional()])
    price = FloatField('Price, Figures only e.g 16.99', validators=[Optional()])
    ...
    lead_time = StringField('Delivery time in Hours or Days')
